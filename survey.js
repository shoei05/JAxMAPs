...
(function () {
  ...
  function restrictedQuestionMarkup(q, detailed = false) {
    ...
    return `<${detailed ? 'div class="sv-dialog-heading"' : `article class="sv-question" data-sv-card="${esc(q.id)}"`}><div class="sv-question-meta"><span class="sv-study-pill">${esc(q.study)}</span><span>${q.substudy === 'pregnancy' ? esc((((waveById && waveById.get(q.wave_id)) || {}).label || q.year + ' 妊産婦票').replace(/^JACSIS\s*/, '')) : esc(q.year) + (q.substudy ? ' · ' + esc(substudyLabel(q.substudy)) : '')}</span><span class="sv-qnumber">${esc(questionNumber(q))}</span></div>${detailed ? `<h2 id="sv-dialog-title">${title}</h2>` : `<h3>${title}</h3>`}<p>本文は公式調査票で確認</p>${url ? `<a class="sv-detail-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">調査票を開く <span aria-hidden="true">↗</span></a>` : ''}</${detailed ? 'div' : 'article'}>`;
  }
